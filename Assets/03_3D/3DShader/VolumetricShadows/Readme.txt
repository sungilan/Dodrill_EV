...
Use Height Bias - Enabling this boolean will allow the shadow's Y centerpoint to be
    "biased" up or down. This is generally intended to be used by biasing the shadow
    nearly to the top, scaling the shadow geometry up a lot in Y, and locking the shadow
    to the feet of a character. This will allow it to follow the character even when
    jumping without needing a constant raycast downward to place the shadow on the
    ground.

Height Bias - This slider blends the Y centerpoint of the shadow's shape up or down
    inside the shadow's volume. 0.499 will place it a the top, and -0.499 at the bottom.

Height Power - This controls the curve of the shadow's fade along the Y axis. A large
    value will cause the shadow to fade slowly along most of the length and then quickly
    near the extremities. A small value will cause the shadow to fade away quickly near
    the Y center of the shadow.
