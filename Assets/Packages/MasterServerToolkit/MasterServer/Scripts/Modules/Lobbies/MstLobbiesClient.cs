...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class MstLobbiesClient : MstBaseClient
    {
        ...
        public void CreateAndJoin(string factory, MstProperties options, JoinLobbyCallbackHandler callback, string platformType)
        {
        ...
        /// <param name="callback"></param>
        public void JoinLobby(int lobbyId, JoinLobbyCallbackHandler callback, string platformType)
        {
            JoinLobby(lobbyId, callback, Connection, platformType);
        }

        /// <summary>
        /// Sends a request to join a lobby
        /// </summary>
        public void JoinLobby(int lobbyId, JoinLobbyCallbackHandler callback, IClientSocket connection, string platformType)
        {
            // Send the message
            connection.SendMessage(MstOpCodes.JoinLobby, lobbyId, (status, response) =>
            {
                ...
                //joinedLobby.Members[Mst.Client.Auth.AccountInfo.Username].Properties.Add(Mst.Args.Names.PlatformType, platformType);


                //var options = joinedLobby.Members[Mst.Client.Auth.AccountInfo.Username].Properties;
                //options.Add(Mst.Args.Names.PlatformType, platformType);
                //SetMyProperties(options, (h, d) =>
                //{
